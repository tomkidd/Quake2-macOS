...
typedef struct
{
	...
	qboolean	have_stencil; // Knightmare- stencil buffer
...
} glconfig_t;extern	glconfig_t gl_config;
...
static void CIN_ReadInfo (cinematic_t *cin)
{
	...
	cin->rawWidth = Clamp(cin->vidWidth, 1, gl_config.max_texsize); // was 512
	cin->rawHeight = Clamp(cin->vidHeight, 1, gl_config.max_texsize); // was 512
