 ...
 */
void
Use_Target_Tent(edict_t *self, edict_t *other /* unused */, edict_t *activator /* unused */)
{
	if (!self)
	{
		return;
...
void
Use_Target_Help(edict_t *self, edict_t *other /* unused */, edict_t *activator /* unused */)
{
	if (!self)
	{
		return;
...
SP_target_spawner(edict_t *self)
{
    ...
    self->class_id = ENTITY_TARGET_SPAWNER;
...
use_target_blaster(edict_t *self, edict_t *other /* unused */, edict_t *activator /* unused */)
{
    ...
    // this was removed by yq2 previously -tkidd
    ...
    {
        fire_grenade(self, start, movedir, self->dmg, self->speed, 2.5, self->dmg+40);
...
void use_target_change (edict_t *self, edict_t *other, edict_t *activator)
{
    ...
    while(target_ent)
    {
        ...
        if(self->spawnflags)
        {
            ...
            // special cases:
            if(target_ent->class_id == ENTITY_MODEL_TRAIN)
            {
...
void clone (edict_t *self, edict_t *other, edict_t *activator)
{
    ...
    if(child->class_id == ENTITY_FUNC_BUTTON)
    {
    ...
    else if(child->class_id == ENTITY_FUNC_DOOR)
    {
        VectorCopy(child->s.origin,child->pos1);
        child->moveinfo.distance = parent->moveinfo.distance;
        VectorMA(child->pos1, child->moveinfo.distance, child->movedir, child->pos2);
    ...
    else if(child->class_id == ENTITY_FUNC_DOOR_ROTATING)
    {
    ...
    else if(child->class_id == ENTITY_FUNC_ROTATING)
    {
        VectorClear(child->s.angles);
        if (child->spawnflags & 1)
            child->use (child, NULL, NULL);
    }
    else if(child->class_id == ENTITY_FUNC_TRAIN)
    {
