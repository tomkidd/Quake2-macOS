...
monster_fire_blaster(edict_t *self, vec3_t start, vec3_t dir, int damage,
		int speed, int flashtype, int effect, int color)
{
	if (!self)
	{
		return;
...
M_WorldEffects(edict_t *ent)
{
	...
	if ((ent->watertype & CONTENTS_SLIME) && !(ent->flags & FL_IMMUNE_SLIME) && !(ent->svflags & SVF_DEADMONSTER) && !(ent->svflags & SVF_DEADMONSTER))
	{
		if (ent->damage_debounce_time < level.time)
		{
...
monster_triggered_spawn(edict_t *self)
{
	...
	if (self->enemy && !(self->spawnflags & SF_MONSTER_SIGHT) &&
